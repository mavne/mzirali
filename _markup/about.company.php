<?php 
$version = "1.0.5";
?>
<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<link rel="icon" href="/_markup/assets/img/favicon.png" type="image/svg" sizes="32x32">
	<title>Mzirali</title>
	<link rel="stylesheet" type="text/css" href="/_markup/assets/bootstrap/css/bootstrap.min.css">
	<link rel="stylesheet" type="text/css" href="/_markup/assets/owl/owl.carousel.min.css">
	<link rel="stylesheet" href="/_markup/assets/lightbox/jquery.lightbox.css">
	<link rel="stylesheet" type="text/css" href="/_markup/assets/css/main.css?v=<?=$version?>">
	<link rel="stylesheet" type="text/css" href="/_markup/assets/css/ge.css?v=<?=$version?>">
	<link rel="stylesheet" type="text/css" href="/_markup/assets/css/responsive_ge.css?v=<?=$version?>">
</head>
<body class="inner-page about-company-page">

<div class="mobile-navigation"></div>
<div class="mobile-header"></div>

<div class="fixed-menu">
	<div class="top">
		<div class="center_small"></div>
	</div>
	<div class="bottom">
		<div class="center_small"></div>
	</div>
</div>

<div class="navigation-box">
	<div class="center">
		<a href="javascript:void(0)" class="close">close</a>

		<div class="nav-box">
			<nav>
				<ul>
					<li><a href="">შესახებ</a></li>
					<li><a href="">ბინის არჩევა</a></li>
					<li><a href="">ადგილმდებარეობა</a></li>
					<li><a href="">საინვესტიციო მიმზიდველობა</a></li>
					<li><a href="">კომპანია</a></li>
					<li><a href="">კონტაქტი</a></li>
					<li><a href="">სიახლეები</a></li>
				</ul>
			</nav>
		</div>
	</div>
</div>

<div class="lines">
	<div class="vertical">
		<div class="left"></div>
		<div class="middle"></div>
		<div class="right"></div>
	</div>

	<div class="horizontal"></div>
</div>

<div class="logo-live-social">
	<div class="center">
		<a href="" class="logo"></a>
		
		<div class="right">
			<div class="center_small">
				<a href="" class="live-video">LIVE</a>
			</div>

			<ul class="social">
				<li><a href="" class="facebook">facebook</a></li>
				<li><a href="" class="instagram">instagram</a></li>
				<li><a href="" class="telegram">telegram</a></li>
				<li><a href="" class="whatsapp">whatsapp</a></li>
				<li><a href="" class="viber">viber</a></li>
			</ul>
		</div>
	</div>
</div>

<header>
	<div class="center">
		<div class="center_small">
			<div class="burger-language-data">
				<a href="javascript:void(0)" class="burger">burger</a>

				<div class="line"></div>

				<ul class="languages">
					<li><a href="" class="active">GE</a></li>
					<li><a href="">EN</a></li>
					<li><a href="">RU</a></li>
				</ul>
			
				<div class="data-box">
					<ul>
						<li>
							<h2>აპარტამენდები</h2>
							<p>46 000-დან</p>
						</li>

						<li>
							<h2>უკუგება</h2>
							<p>5 - 6 წელი</p>
						</li>

						<li>
							<h2>შემოსავალი</h2>
							<p>10 000$ (წლიური)</p>
						</li>
					</ul>
				</div>
			</div>			

			
		</div>

		<div class="contact-box">
			<a href="" class="ask-call">ზარის შეკვეთა</a>
			<a href="" class="call">+995 595 06 23 23</a>
		</div>
	</div>
</header>


<main>
	<section class="cover" style="background-image:url('/_markup/assets/img/cover.jpg')">
		<div class="multi"></div>
		<div class="center_small">
			<h1>კომპანის შესახებ</h1>
		</div>
	</section>

	<section class="about-company">
		<div class="center_small">
			<h2>საქართველოს შესახებ</h2>
			<p>"მზირალის“ მშენებელი და ინვესტორი კომპანია „,მშენბერია“, რომელიც 2013 წლიდან საქმიანობს ბაზარზე საპროექტო, სამშენებლო და სარემონტო სამუშაოების  მიმართულებით. კომპანიის მისიაა ავაშენოთ კომფორტული და ხარისხიანი საცხოვრებელი სახლები, დავიმსახუროთ ყველაზე მთავარი - მომხმარებლის ნდობა, რადგან მიგვაჩნია, რომ ხარისხი და ნდობა წარმატების მთავარი საწინდარია. ჩვენი მიზანია დავნერგოთ ხელმისაწვდომი, ხარისხიანი და კომფორტული ცხოვრების სტანდარტი. კომპანიისთვის პრიორიტეტულია მშენებლობის ვადების ზუსტი დაცვა და მაღალი ხარისხის უზრუნველყოფა. </p>
			<p>კომპანიას უკვე წარმატებით აქვს განხორციელებული 2 დეველოპერული პროექტი ბათუმში. ლოგისტიკური და ინფრასტრუქტურული განვითარების  კვალდაკვალ კომპანია ახალ პროექტს გონიოში ახორციელებს.</p>

			<div class="image-list">
				<div class="row">
					<div class="col-md-3">
						<div class="image-box">
							<div class="image" style="background-image:url('/_markup/assets/img/dest.jpg')"></div>
						</div>
					</div>

					<div class="col-md-3">
						<div class="image-box">
							<div class="image" style="background-image:url('/_markup/assets/img/dest.jpg')"></div>
						</div>
					</div>

					<div class="col-md-3">
						<div class="image-box">
							<div class="image" style="background-image:url('/_markup/assets/img/dest.jpg')"></div>
						</div>
					</div>

					<div class="col-md-3">
						<div class="image-box">
							<div class="image" style="background-image:url('/_markup/assets/img/dest.jpg')"></div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>
</main>

<footer>
	<div class="map"></div>

	<div class="center_small topMinus25">
		<div class="data">
			<div class="multi"></div>
			<ul>
				<li><span>ბათუმის საერთაშორისო<br>აეროპორტი 6კმ.</span></li>
				<li><span>ქუთაისის საერთაშორისო<br>ეროპორტი 129 კმ.</span></li>
				<li><span>რიზე-ართვინის აეროპორტი<br>84 კმ.</span></li>
				<li><span>ტრაბზონის აეროპორტი<br>188 კმ.</span></li>
			</ul>
		</div>
	</div>

	<div class="center">
		<h3>საქართველო, გონიო</h3>

		<div class="contact-data">
			<div class="multi"></div>
			<div class="center_small">
				<div class="social">
					<ul>
						<li><a href="" class="facebook">facebook</a></li>
						<li><a href="" class="instagram">instagram</a></li>
					</ul>
				</div>

				<div class="messages">
					<ul>
						<li><a href="" class="whatsapp icon">whatsapp</a></li>
						<li><a href="" class="viber icon">viber</a></li>
						<li><a href="" class="call">+995 595 06 23 23</a></li>
					</ul>
				</div>

				<div class="mailto">
					<a href="" class="mail">mzirali@gmail.com</a>
				</div>

				<div class="ask-call-box">
					<a href="" class="ask-call">ზარის მოთხოვნა</a>
				</div>
			</div>
		</div>

		<div class="copyright">
			<div class="box">
				<p>ყველა უფლება დაცულია ©2023</p>

				<a href="" class="shindi">shindi</a>
			</div>
		</div>
	</div>
</footer>



<script src="/_markup/assets/js/jquery.js"></script>
<script src="/_markup/assets/lightbox/jquery.lightbox.js"></script>
<script src="/_markup/assets/bootstrap/js/bootstrap.min.js"></script>
<script src="/_markup/assets/js/owl.carousel.min.js"></script>
<script src="/_markup/assets/js/compressed.js?v=<?=$version?>"></script>
<script>
	$('#chooseFlatOwl').owlCarousel({
	    loop:true,
	    margin:15,
	    nav:true,
	    responsive:{
	        0:{
	            items:1
	        },
	        600:{
	            items:3
	        },
	        1000:{
	            items:3
	        }
	    }
	});
</script>
</body>

</html>